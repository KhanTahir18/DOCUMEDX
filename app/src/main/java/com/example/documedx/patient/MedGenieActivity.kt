...
class MedGenieActivity : AppCompatActivity() {
    ...
    private fun setupGemini() {
        ...
        if (apiKey.isEmpty() || apiKey == "your_api_key_here") {
            Toast.makeText(this, "Please add your Gemini API key to strings.xml", Toast.LENGTH_LONG).show()
            return
        }
    ...
    private fun getGeminiResponse(userMessage: String) {
        lifecycleScope.launch {
            try {
                ...
                // Debug logging
                println("MedGenie: Sending prompt to Gemini API")
                println("MedGenie: API Key exists: ${getString(R.string.gemini_api_key).isNotEmpty()}")
                ...
                println("MedGenie: Successfully got response from Gemini")

            } catch (e: Exception) {
                binding.progressLoading.visibility = View.GONE

                // Debug logging for the actual error
                println("MedGenie Error: ${e.message}")
                println("MedGenie Error Details: ${e.printStackTrace()}")

                val errorMessage = ChatMessage(
                    "Error: ${e.message ?: "Unknown error occurred"}. Please check your API key and internet connection.",
                    isUser = false
                )
